...
import { LayoutDashboard, PieChart, LogOut, Plus, Briefcase } from "lucide-react";
    ...
    <div className="flex h-screen w-full bg-background text-on-surface overflow-hidden relative font-sans selection:bg-primary/30 selection:text-white">
      ...
      <div className="bg-ornament bg-blob-tertiary opacity-10 blur-[180px] -bottom-40" />
          ...
          <main className="flex-1 flex flex-col relative min-w-0 overflow-y-auto no-scrollbar pb-32 md:pb-0 z-0">
                ...
                <Link href="/settings" className="flex items-center hover:scale-[1.02] transition-all active:scale-95 group/profile">
                  <div className="flex items-center gap-4 px-6 py-2.5 rounded-[2rem] glass-card backdrop-blur-2xl border-white/10 bg-white/[0.02] shadow-2xl relative overflow-hidden">
                    ...
                    <div className="relative group/avatar flex-shrink-0">
                      ...
                      />
                      <div className="absolute -bottom-1 -right-1 h-3.5 w-3.5 rounded-full bg-secondary border-[3px] border-surface z-30 shadow-glow-secondary shadow-black/50" />
                    </div>
                  </div>
                </Link>
              )}
            ...
            <div className="flex-1 px-4 pt-4 md:px-12 md:pt-2 lg:px-16 lg:pt-2 animate-in fade-in slide-in-from-bottom-2 duration-1000 relative">
              {children}
              
              {/* Client-side Auth Overlay */}
              {(loading || !user) && (
                <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-background/60 backdrop-blur-xl">
                  ...
                  <p className="mt-4 text-[10px] font-black uppercase tracking-[0.3em] text-on-surface-variant/40 animate-pulse">
                    Authenticating
                  </p>
                </div>
              )}
